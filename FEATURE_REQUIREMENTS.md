...
| FR-08 | Mentor matching | ✅ Done (2026-06-04) | Weighted scoring (topic 35%, track 25%, logistics 15%, **behavioral DISC 15%**, stage 10%). DISC compatibility scoring (S complementarity, D alignment, C/I bonuses) via cached profiles. Queue length + estimated wait time from live session/request data. `hourlyRate` + explainable `reasons` displayed on match cards. Request Session buttons fully functional with pricing. 63 tests (8 behavioral-specific). | Semantic/embedding matching (V2); feedback loop from session ratings (V2) |
...
**7 of 8 features complete:** FR-01 (resume+review), FR-02 (5D+history+shifts), FR-03 (AI questions+agenda), FR-04 (video via Daily.co), FR-05 (conversations), FR-07 (billing+Stripe), FR-08 (matching+DISC compatibility). **Remaining:** FR-06 end-to-end wiring (chat events → behavioral backend DISC recompute).
